...
const ArtistSearch = ({ artistSearch, citySearch }: ArtistSearchProps) => {
  ...
  useEffect(() => {
    ...
    const fetchArtists = async () => {
      try {
        const response = await fetch(`/api/artists/?artistSearch=${searchTerm}`)
  ...
  useEffect(() => {
    ...
    if (citySearch != '') {
      const fetchGigCity = async () => {
        try {
          const response = await fetch(`/api/andCity/?id=${id}&city=${citySearch}`)
          const data = await response.json()
      ...
      <div className='grid grid-cols-1 justify-items-center lg:grid-cols-3  md:grid-cols-2 gap-3 px-3'>
        ...
        {searchTerm != '' && gigs.length === 0 && (
          <div className='col-span-full px-3 py-2 m-6 rounded-md border text-md text-center'>
            Sorry, no gigs were found for this artist! <br /> Try removing the city if you added one or try another
            artist.
          </div>
        )}
      </div>
